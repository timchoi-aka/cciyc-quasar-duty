...
export const EVENT_REREGISTRATION = gql`
  mutation EventRegistration(
    $logObject: Log_insert_input! = {}
    $accountObject: tbl_account_insert_input = {}
  ) {
...
export const DELETE_EVENT_STAT = gql`
  mutation deleteEventStat(
    ...
    delete_tbl_act_session(
      where: {
        c_act_code: { _in: $delete_cActCode }
        d_act: { _in: $delete_dAct }
        inCenter: { _in: $delete_inCenter }
      }
    ) {
      affected_rows
    }
...
export const ADD_FAVOURATE = gql`
  mutation addFavourate($c_act_code: String = "", $username: String = "") {
    insert_Event_Favourate_one(
      object: { c_act_code: $c_act_code, username: $username }
    ) {
      username
      c_act_code
    }
