    ...
    <q-card class="col-5">
      <q-card-section class="row content-start">
        <div class="col-6 row content-start">
          ...
          <q-card square class="col-11">
            ...
            <q-card-section>
              <q-input
                v-if="MemberObject.c_mem_id == '9999'"
                v-model="MemberObject.c_name"
              />
      ...
      <q-card-actions class="row">
        <div class="q-mt-sm text-h6 q-mx-sm col-7 row">
          <span class="col-4">總數：$</span
          ><q-input type="number" v-model="totalAmount" class="col-8" />
        </div>
        <q-space />
        <q-btn
          label="重置"
          icon="restart_alt"
          class="bg-warning text-white q-mx-sm col-2"
          @click="reset"
        />
        <q-btn
          label="確認"
          icon="check"
          class="bg-positive text-white q-mx-sm col-2"
          @click="save"
        />
      </q-card-actions>
    </q-card>
...
const {
  mutate: addOtherIncome,
  onDone: addOtherIncome_Completed,
  onError: addOtherIncome_Error,
} = useMutation(gql`
...
function updateAmount() {
  totalAmount.value = incomeType.value
    ? incomeType.value.u_fee * quantity.value
    : 0;
}
...
function save() {
  ...
  const logObject = ref({
    ...
    action:
      ...
      ")" +
      MemberObject.value.c_name
        ? MemberObject.value.c_name
        : MemberObject.value.c_name_other +
          "，收費項目：" +
          incomeType.value.value +
          "（$" +
          incomeType.value.u_fee +
          ") 數目：" +
          quantity.value +
          " 總數：" +
          quantity.value * incomeType.value.u_fee +
          " 收據號碼：" +
  ...
  const remark =
    ...
    quantity.value +
    " X HK$" +
    incomeType.value.u_fee.toFixed(2) +
    "\r";
  const accountObject = ref({
    ...
    c_act_code: null,
    c_type: incomeType.value.c_acc_type
      ? incomeType.value.c_acc_type.trim()
      : "",
    u_discount: 0,
    ...
    c_mem_id: MemberObject.value.c_mem_id
      ? MemberObject.value.c_mem_id.trim()
      : "",
    c_user_id: username,
    c_name: MemberObject.value.c_name
      ? MemberObject.value.c_name.trim()
      : MemberObject.value.c_name_other.trim(),
    b_cssa: false,
    b_refund: false,
    b_OtherIncome: true,
    b_clear: false,
...
addOtherIncome_Completed((result) => {
  ...
  $q.notify(
    {
      progress: true,
      message: "收費完成。收據編號：" + receiptNumber,
      actions: [
        {
          label: "列印收據",
          color: "yellow",
