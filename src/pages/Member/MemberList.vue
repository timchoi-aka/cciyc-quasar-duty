  ...
  <q-dialog v-model="modalDisplay"
      ...
      <q-card class="q-pa-md">
        <q-card-section class="bg-primary text-white row text-h6">
          <div class="col-*">
            {{ modalObject.c_mem_id }} 
            - 
            <span v-if="modalObject.c_name != null">{{modalObject.c_name}}</span>
            <span v-if="modalObject.c_name_other != null">({{modalObject.c_name_other}})</span>
            <span>[{{modalObject.c_sex}}]</span>
          </div>
          <q-space/>
        ...
        <q-card-section class="row text-h6">
          <div class="col-2">出生日期<br/>{{qdate.formatDate(modalObject.d_birth, "YYYY年MM月DD日")}}</div>
          <div class="col-1">年齡<br/>{{calculateAge(modalObject.d_birth)}}</div>
          <div class="col-2">手提電話<br/>{{modalObject.c_mobile}}</div>
          <div class="col-2">家居電話<br/>{{modalObject.c_tel}}</div>
          <div class="col-2">地址<br/>{{modalObject.m_addscom}}</div>
          <div class="col-2">電郵<br/>{{modalObject.c_email}}</div>
        </q-card-section>
        <q-card-section class="row text-h6">
          <div class="col-2">緊急聯絡人<br/>{{modalObject.c_emer_name}}</div>
          <div class="col-2">關係<br/>{{modalObject.c_emer_rel}}</div>
          <div class="col-2">電話<br/>{{modalObject.c_emer_tel1_1}}</div>
        </q-card-section>
        <q-card-section class="row text-h6">
          <div class="col-2">會籍<br/>{{modalObject.c_udf_1}}
            (<q-icon v-if="modalObject.b_mem_type1_valid" color="positive" name="check"/>
            <q-icon v-else color="negative" name="cancel"/>)
          </div>
          <div class="col-2">入會日期<br/>{{qdate.formatDate(modalObject.d_enter_1, "YYYY年MM月DD日")}}</div>
          <div class="col-2">屆滿日期<br/>{{qdate.formatDate(modalObject.d_expired_1, "YYYY年MM月DD日") == "3000年01月01日"? "永久":qdate.formatDate(modalObject.d_expired_1, "YYYY年MM月DD日")}}</div>
          <div class="col-2">續會日期<br/>{{qdate.formatDate(modalObject.d_renew_1, "YYYY年MM月DD日")}}</div>
          <div class="col-2">退會日期<br/>{{qdate.formatDate(modalObject.d_exit_1, "YYYY年MM月DD日")}}</div>
          <div class="col-1">青年家人<br/>
            <q-icon v-if="modalObject.b_mem_type_10" color="positive" name="check"/>
            <q-icon v-else color="negative" name="cancel"/>
          </div>
        </q-card-section>
        <q-card-section class="row text-h6">
          <div class="col-2">更新日期<br/>{{qdate.formatDate(modalObject.d_update, "YYYY年MM月DD日")}}</div>
          <div class="col-2">更新職員<br/>{{modalObject.c_update_user}}</div>
        </q-card-section>
      </q-card>
  </q-dialog>
    ...
    try {
      ...
      */
      ...
      memberResponse.data.data.Member.forEach((record) => {
        qdate.getDateDiff(this.today, record.d_expired_1) < 0 ? record.b_mem_type1_valid = true : record.b_mem_type1_valid = false;
        qdate.getDateDiff(this.today, record.d_expired_2) < 0 ? record.b_mem_type2_valid = true : record.b_mem_type2_valid = false;
        qdate.getDateDiff(this.today, record.d_expired_3) < 0 ? record.b_mem_type3_valid = true : record.b_mem_type3_valid = false;
        qdate.getDateDiff(this.today, record.d_expired_4) < 0 ? record.b_mem_type4_valid = true : record.b_mem_type4_valid = false;
