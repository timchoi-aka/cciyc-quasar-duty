<template>
  ...
  <div class="row justify-center">
    <div class="row items-center q-mx-md">
      <q-btn
        label="上月"
        @click="
          reportDate = qdate.formatDate(
            qdate.endOfDate(
              qdate.subtractFromDate(reportDate, { month: 1 }),
              'month'
            ),
            'YYYY/MM/DD'
          )
    ...
    <div>
      <q-input filled v-model="reportDate" mask="date" :rules="['date']">
        <template v-slot:prepend> 截數月份： </template>
    ...
    <div class="row items-center q-mx-md">
      <q-btn
        ...
        @click="
          reportDate = qdate.formatDate(
            qdate.endOfDate(qdate.addToDate(reportDate, { month: 1 }), 'month'),
            'YYYY/MM/DD'
          )
        "
        class="bg-primary text-white items-center"
  ...
  >
    <q-tab
      name="All"
      icon="source"
      :label="'全部(' + MemberData.length + '人)'"
    />
    <q-tab
      name="Youth"
      icon="pin_drop"
      :label="'青年(' + YouthData.length + '人)'"
    />
    <q-tab
      name="Family_15"
      icon="pin_drop"
      :label="'家人(<15)(' + Family_15Data.length + '人)'"
    />
    <q-tab
      name="Family_24"
      icon="pin_drop"
      :label="'家人(>24)(' + Family_24Data.length + '人)'"
    />
    <q-tab
      name="Quit"
      icon="pin_drop"
      :label="'退會(' + QuitData.length + '人)'"
    />
    <q-tab
      name="Expired"
      icon="pin_drop"
      :label="'截數月過期(' + ExpiredData.length + '人)'"
    />
    <q-tab
      name="Error"
      icon="error"
      :label="'錯誤(' + ErrorData.length + '人)'"
    />
    <q-tab
      name="Duplicate"
      icon="error"
      :label="'重覆(' + DuplicateData.length + '人)'"
    />
  </q-tabs>
          ...
          <q-btn
            ...
            @click="
              exportExcel(
                YouthData,
                memberListColumns,
                '青年會員數據_' + qdate.formatDate(reportDate, 'YYYY-MM')
              )
            "
          />
        </template>
          ...
          <q-btn
            ...
            @click="
              exportExcel(
                ...
                memberListColumns,
                '家人(14歲或以下)會員數據_' +
                  qdate.formatDate(reportDate, 'YYYY-MM')
              )
            "
          />
        </template>
          ...
          <q-btn
            ...
            @click="
              exportExcel(
                ExpiredData,
                memberListColumns,
                '過期會員數據_' + qdate.formatDate(reportDate, 'YYYY-MM')
              )
            "
          />
        </template>
      ...
      >
        <template v-slot:top-right>
          <q-btn
            ...
            no-caps
            @click="
              exportExcel(DuplicateData, memberListColumns, '重覆會員數據')
            "
          />
        </template>
      </q-table>
...
function updateReport() {
  ...
  const { youthMembers, childMembers, familyMembers } = Report.classifyMember(
    reportDate,
    res
  );
  ...
  ErrorData.value = res.filter(
    (member) =>
      ...
      (member.d_expired_1 == null ||
        (member.d_expired_1 &&
          qdate.getDateDiff(member.d_expired_1, reportDate.value) > 0))
  );
  ...
  res.forEach((x) => {
    ...
    if (
      res.filter(
        (member) =>
          member.c_name == x.c_name &&
          member.d_birth == x.d_birth &&
          qdate.extractDate(member.d_expired_1, "YYYY-MM-DDTHH:mm:ss") > now &&
          qdate.extractDate(x.d_expired_1, "YYYY-MM-DDTHH:mm:ss") > now
      ).length > 1
    ) {
