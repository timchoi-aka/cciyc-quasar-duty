...
<div>
  ...
  <q-table flat :grid="$q.screen.lt.sm" :rows="users" :columns="tableFields" :hide-bottom="true"
    :pagination="pagination" color="primary" row-key="uid">
    <!-- grid template -->
        ...
        <q-card class="q-pa-none">
          <q-card-section class="bg-blue-1 q-mb-md row justify-around items-center">
            ...
            <div class="col-sm-6 q-mx-sm text-body1">
              排序{{ props.row.order }}
              <q-btn :disable="props.row.order == 1" round size="xs" color="positive" icon="keyboard_arrow_up"
                @click="changeOrder(props.key, 'UP')" />
              <q-btn :disable="props.row.order == users.length" round size="xs" color="negative"
                icon="keyboard_arrow_down" @click="changeOrder(props.key, 'DOWN')" />
            </div>
          </q-card-section>
          <q-card-section style="font-size: 1vw" class="row justify-around q-pa-none">
            <div class="col-xs-3 justify-center" style="text-align: center">
              ...
              <q-btn round :color="props.row.enable ? 'positive' : 'negative'" :label="props.row.enable ? '有' : '沒有'"
                @click="changeEnable(props.key)" />
            </div>
            <div class="col-xs-3 justify-center" style="text-align: center">
              <div class="text-caption">批核假期</div>
              <q-btn round :color="props.row.privilege_leaveApprove ? 'positive' : 'negative'"
                :label="props.row.privilege_leaveApprove ? '有' : '沒有'" @click="changeLeaveApprove(props.key)" />
            </div>
            <div class="col-xs-3 justify-center" style="text-align: center">
              <div class="text-caption">檢視假期</div>
              <q-btn round :color="props.row.privilege_leaveManage ? 'positive' : 'negative'"
                :label="props.row.privilege_leaveManage ? '有' : '沒有'" @click="changeLeaveManage(props.key)" />
            </div>
            <div class="col-xs-3 justify-center" style="text-align: center">
              <div class="text-caption">特別年假</div>
              <q-btn round :color="props.row.privilege_sal ? 'positive' : 'negative'"
                :label="props.row.privilege_sal ? '有' : '沒有'" @click="changeSal(props.key)" />
            </div>
            <div class="col-xs-3 justify-center" style="text-align: center">
              <div class="text-caption">修改更表</div>
              <q-btn round :color="props.row.privilege_scheduleModify ? 'positive' : 'negative'"
                :label="props.row.privilege_scheduleModify ? '有' : '沒有'" @click="changeScheduleModify(props.key)" />
            </div>
            <div class="col-xs-3 justify-center" style="text-align: center">
              <div class="text-caption">用戶管理</div>
              <q-btn round :color="props.row.privilege_userManagement ? 'positive' : 'negative'"
                :label="props.row.privilege_userManagement ? '有' : '沒有'" @click="changeUserManagement(props.key)" />
            </div>
            <div class="col-xs-3 justify-center" style="text-align: center">
              <div class="text-caption">試用</div>
              <q-btn round :color="props.row.privilege_probation ? 'positive' : 'negative'"
                :label="props.row.privilege_probation ? '有' : '沒有'" @click="changeProbation(props.key)" />
            </div>
            <div class="col-xs-3 justify-center" style="text-align: center">
              <div class="text-caption">財務</div>
              <q-btn round :color="props.row.privilege_finance ? 'positive' : 'negative'"
                :label="props.row.privilege_finance ? '有' : '沒有'" @click="changeFinance(props.key)" />
            </div>
            <div class="col-xs-3 justify-center" style="text-align: center">
              <div class="text-caption">活動管理</div>
              <q-btn round :color="props.row.privilege_eventManagement ? 'positive' : 'negative'"
                :label="props.row.privilege_eventManagement ? '有' : '沒有'" @click="changeEventManagement(props.key)" />
            </div>
            <div class="col-xs-3 justify-center" style="text-align: center">
              <div class="text-caption">計劃審批</div>
              <q-btn round :color="props.row.privilege_eventApprove ? 'positive' : 'negative'"
                :label="props.row.privilege_eventApprove ? '有' : '沒有'" @click="changeEventApprove(props.key)" />
            </div>
            <div class="col-xs-3 justify-center" style="text-align: center">
              <div class="text-caption">審批醫療</div>
              <q-btn round :color="props.row.privilege_healthapprove ? 'positive' : 'negative'"
                :label="props.row.privilege_healthapprove ? '有' : '沒有'" @click="changeHealthApprove(props.key)" />
            </div>
            <div class="col-xs-3 justify-center" style="text-align: center">
              <div class="text-caption">資產管理</div>
              <q-btn round :color="props.row.privilege_inventoryManagement ? 'positive' : 'negative'"
                :label="props.row.privilege_inventoryManagement ? '有' : '沒有'"
                @click="changeInventoryManagement(props.key)" />
            </div>
          </q-card-section>
          ...
          <q-card-section class="q-pa-none q-pb-sm">
            ...
            <q-btn v-if="!editEmployment" label="修改" icon="edit" class="bg-primary text-white q-my-md"
              @click="startEditEmployment(props.key)" />
            <q-btn v-if="editEmployment" label="新增" icon="add" class="bg-primary text-white q-my-md"
              @click="newEmploymentRecord.push({ dateOfEntry: null, dateOfExit: null, rank: null })" />
            <q-btn v-if="editEmployment" label="取消" icon="cancel" class="bg-negative text-white q-my-md"
              @click="cancelEmploymentRecordChange" />
            <q-btn v-if="editEmployment && JSON.stringify(newEmploymentRecord) != JSON.stringify(props.row.employment)"
              label="儲存" icon="save" class="bg-positive text-white q-my-md" @click="saveEmploymentRecord(props.key)" />
            ...
            <div v-else v-for="(employ, index) in newEmploymentRecord" class="row">
              <div class="col-4">入職日期：</div>
              <div class="col-8">
                <DateComponent v-model="employ.dateOfEntry" label="人職日期" />
              </div>
              <div class="col-4">離職日期：</div>
              <div class="col-8">
                <DateComponent v-model="employ.dateOfExit" label="離職日期" />
              </div>
              <div class="col-4">職級：</div>
              <div class="col-4">
                <q-select :label="rankInputReverseMap[newEmploymentRecord[index].rank]" hide-bottom-space
                  v-model="newEmploymentRecord[index].rank" :options="rankInputOptions" />
              </div>
              <q-btn v-if="index > 0" class="bg-negative text-white" label="刪除" icon="delete"
                @click="newEmploymentRecord.splice(index, 1)" />
            </div>
          </q-card-section>
          <q-card-section class="row justify-around items-center q-pa-none q-pb-sm"><q-chip size="lg"
              class="bg-primary text-white" label="預設更表" />
            <div class="col-12 row">
              <span class="text-center column col-2" v-for="(item, index) in props.row.defaultSchedule">
                <div>{{ scheduleIndex[index] }}</div>
                <div>
                  <q-chip :label="item">
                    <q-popup-edit filled v-model="props.row.defaultSchedule[index]" :title="scheduleIndex[index]"
    ...
    <template v-slot:body="props">
      <q-tr :props="props">
        ...
        <!-- extra info button -->
        <q-td style="font-size: 1.2vw; text-align: center; width: 3vw;">
          <q-btn color="primary" round dense @click="props.expand = !props.expand"
        ...
        <q-td colspan="100%">
          ...
          <q-tab-panels v-model="activeTab" animated swipeable transition-prev="jump-up" transition-next="jump-up">
            <q-tab-panel name="employment" class="q-ma-none q-pa-sm text-body1">
              <q-btn v-if="!editEmployment" label="修改" icon="edit" class="bg-primary text-white q-my-md"
                @click="startEditEmployment(props.key)" />
              <q-btn v-if="editEmployment" label="新增受聘記錄" icon="add" class="bg-primary text-white q-my-md"
                @click="newEmploymentRecord.push({ dateOfEntry: null, dateOfExit: null, rank: null })" />
              <q-btn v-if="editEmployment" label="取消修改" icon="cancel" class="bg-negative text-white q-my-md"
                @click="cancelEmploymentRecordChange" />
              <q-btn
                v-if="editEmployment && JSON.stringify(newEmploymentRecord) != JSON.stringify(props.row.employment)"
                label="儲存" icon="save" class="bg-positive text-white q-my-md"
                @click="saveEmploymentRecord(props.key)" />
    ...
    <template v-slot:body-cell-rank="props">
      <q-td :props="props">
        <q-select :label="rankInputReverseMap[props.row.rank]" hide-bottom-space v-model="props.newRank"
          :options="rankInputOptions" @update:model-value="(val) => {
    ...
    <template v-slot:body-cell-dateOfEntry="props">
      <q-td :props="props">
        <span v-html="props.row.dateOfEntry
          ? qdate.formatDate(props.row.dateOfEntry, 'YYYY-MM-DD')
          : ''
          " />
        <q-btn icon="event" round color="primary" class="q-mx-xs">
          <q-popup-proxy @before-show="proxyDate = props.row.dateOfEntry" cover transition-show="scale"
            transition-hide="scale">
            <q-date v-model="proxyDate">
              <div class="row items-center justify-end q-gutter-sm">
                <q-btn label="取消" color="primary" flat v-close-popup />
                <q-btn label="確定" color="primary" flat @click="changeDateOfEntry(props.row.uid, proxyDate)"
                  v-close-popup />
              </div>
            </q-date>
          </q-popup-proxy>
    ...
    <template v-slot:body-cell-dateOfExit="props">
      <q-td :props="props">
        <span v-html="props.row.dateOfExit
          ? qdate.formatDate(props.row.dateOfExit, 'YYYY-MM-DD')
          : ''
          " />
        <q-btn icon="event" round color="primary" class="q-mx-xs">
          <q-popup-proxy @before-show="proxyDate = props.row.dateOfExit" cover transition-show="scale"
            transition-hide="scale">
            <q-date v-model="proxyDate">
              <div class="row items-center justify-end q-gutter-sm">
                <q-btn label="取消" color="primary" flat v-close-popup />
                <q-btn label="確定" color="primary" flat @click="changeDateOfExit(props.row.uid, proxyDate)"
                  v-close-popup />
              </div>
            </q-date>
          </q-popup-proxy>
...
const tableFields = ref([
  ...
  {
    name: "privilege_inventoryManagement",
    label: "資產管理",
    field: "privilege_inventoryManagement",
    headerStyle:
      "font-size: 1.5vw; text-align: center; width: 3vw; max-width: 3vw;",
    style: "font-size: 1.2vw; text-align: center; width: 3vw; max-width: 3vw;",
  },
...
function changeInventoryManagement(uid) {
  // call https functions to change sal privilege
  const toggleInventoryManagement = httpsCallable(FirebaseFunctions, "user-toggleInventoryManagement");
  loading.value++;
  toggleInventoryManagement(uid).then((result) => {
    users.value[users.value.findIndex((value) => value.uid == uid)].privilege_inventoryManagement =
      result.data;
    loading.value--;
  });
}
  ...
  userDoc.forEach((user) => {
    ...
    */
    ...
    users.value.push({
      ...
      privilege_inventoryManagement: d.privilege.inventoryManagement ? d.privilege.inventoryManagement : false,
      order: d.order,
