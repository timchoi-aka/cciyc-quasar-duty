    ...
    <div v-if="$q.screen.gt.sm" class="row fit">
      <div class="col-4 row items-start">
        <q-form @submit="apply" class="col-12 row items-center">
          <div class="col-6">
            <DateComponent v-model="healthCareObject.date" label="收據日期" />
          </div>
          <div class="col-4">
            <q-input
              v-model="healthCareObject.amount"
              label="申領金額"
              type="number"
              :rules="[
                (val) =>
                  (val > 0 && val <= maxRemaining) ||
                  '申領金額 0-' + maxRemaining,
              ]"
            />
          </div>
        ...
        <div class="col-12 items-start">
          <q-table
            ...
            @row-click="
              (event, row, index) =>
                event.target.nodeName === 'I' ? '' : (printObject = row)
            "
          >
            <template v-slot:body-cell-status="props">
              <q-td :props="props">
                <q-icon
                  v-if="props.row.status == '批准'"
                  class="text-positive"
                  name="check"
                >
...
const maxRemaining = computed(() =>
  maxAmount.value -
    healthcare.value.reduce(
      (a, b) => (b.status == "批准" ? a + b.amount : a),
      0
    ) >
  200
    ? 200
...
const healthcareDocQuery = query(
  healthcareCollection,
  where("uid", "==", uid.value),
...
healthcareSnapshot.value = onSnapshot(healthcareDocQuery, (snapshot) => {
  snapshot.docChanges().forEach((change) => {
    ...
    if (change.type == "added") {
      ...
      healthcare.value.splice(
        healthcare.value.findIndex((e) => e.id === change.doc.id),
        1
      );
    }
  });
...
function apply() {
  ...
  const applyHealthCare = httpsCallable(
    FirebaseFunctions,
    "healthcare-applyHealthCare"
  );
  awaitServerResponse.value++;
  applyHealthCare({
    date: qdate.formatDate(
      qdate.extractDate(healthCareObject.value.date, "YYYY/MM/DD"),
      "YYYY-MM-DDT00:00:00"
    ),
...
function deleteApplication(id) {
  const deleteHealthCare = httpsCallable(
    FirebaseFunctions,
    "healthcare-deleteHealthCare"
  );
  awaitServerResponse.value++;
  deleteHealthCare({
    id: id,
