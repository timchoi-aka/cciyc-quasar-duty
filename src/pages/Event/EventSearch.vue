<template>
  <q-page class="row">
    <div class="col-2" style="border: 1px solid">
      <q-form @submit="submitSearch" @reset="clearSearch">
        <div class="row">
          <q-btn
            type="submit"
        ...
        <div class="column">
          <div>
            <q-input clearable label="活動編號" v-model="search.c_act_code" />
          </div>
          <div>
            <q-input clearable label="活動名稱" v-model="search.c_act_name" />
          </div>
          <div>
            <q-select
              clearable
          ...
          <div>
            <q-select
              ...
              label="性質"
              :options="c_nature"
              v-model="search.c_nature"
            />
          </div>
          <div>
            <StaffSelection :multiple="false" v-model="search.c_respon" />
          </div>
          <div>
            <q-select
              clearable
...
const acc_type = ref(["PF", "OF", "MF", "SF", "CF", "RF"]);
...
function submitSearch() {
  let queryString = "";
  if (search.value.c_act_name)
    queryString += "c_act_name=" + search.value.c_act_name + "&";
  if (search.value.c_act_code)
    queryString += "c_act_code=" + search.value.c_act_code + "&";
  if (search.value.c_respon && search.value.c_respon.label != "")
    queryString += "c_respon=" + search.value.c_respon.label + "&";
  if (search.value.c_status)
    queryString += "c_status=" + search.value.c_status + "&";
  if (search.value.c_acc_type)
    queryString += "c_acc_type=" + search.value.c_acc_type + "&";
  if (search.value.c_group1)
    queryString += "c_group1=" + search.value.c_group1 + "&";
  if (search.value.c_nature)
    queryString += "c_nature=" + search.value.c_nature + "&";
...
const {
  result: eventList,
  loading,
  onError: EventListError,
} = useQuery(EVENT_SEARCH, searchCondition);
