<template>
  This is a test page for development options: {{ displayOptions }}
...
const upload_API = process.env.UPLOAD_URL;
const memberID = ref("");
const mobile = ref("");
...
onResult((result) => {
  ...
  result.data.HTX_News.forEach((item) => {
    newsInfo.push({
      ...
      NewsRemark: item.NewsRemark
        ? item.NewsRemark.replace(/[\r\n\t]/g, "").replace(
            /<\/?[^>]+(>|$)/g,
            ""
          )
        : "",
      NewsRemarkEN: item.NewsRemarkEN
        ? item.NewsRemarkEN.replace(/[\r\n\t]/g, "").replace(
            /<\/?[^>]+(>|$)/g,
            ""
          )
        : "",
      NewsContent: item.NewsContent
        ? item.NewsContent.replace(/[\r\n\t]/g, "").replace(
            /<\/?[^>]+(>|$)/g,
            ""
          )
        : "",
      NewsContentEN: item.NewsContentEN
        ? item.NewsContentEN.replace(/[\r\n\t]/g, "").replace(
            /<\/?[^>]+(>|$)/g,
            ""
          )
        : "",
  ...
  result.data.HTX_Article.forEach((item) => {
    articleInfo.push({
      ...
      ArticleContent: item.ArticleContent
        ...
        : "",
      ArticleContentEN: item.ArticleContentEN
        ? item.ArticleContentEN.replace(/[\r\n\t]/g, "").replace(
            /<\/?[^>]+(>|$)/g,
            ""
          )
        : "",
...
async function upload() {
  const upload = httpsCallable(FirebaseFunctions, "file-saveFileToStorage");
  upload()
    .then((result) => {
