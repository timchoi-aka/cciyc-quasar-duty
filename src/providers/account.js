import { ref } from "vue";
import { useLazyQuery, useQuery, useMutation, useSubscription } from "@vue/apollo-composable";
...
export function useAccountProvider(options = {}) {
  ...
  subscriptionResult((res) => {
    if (res.data) {
      let lastReceiptDate = date.extractDate(res.data.tbl_account[0].d_create, "YYYY-MM-DDTHH:mm:ss")
      let today = new Date()
      // test case
      // let today = date.extractDate("2024-04-01T00:00:00", "YYYY-MM-DDTHH:mm:ss")

      let newFinancialYear = lastReceiptDate.getMonth() < 3 && today.getMonth() >= 3
  ...
  const { result, load, loading } = useLazyQuery(GET_ACCOUNT,
    () => ({
  ...
  function buildParam() {
    ...
    if (c_receipt_no.value) condition.value["_and"].push({c_receipt_no: {"_eq" : c_receipt_no.value}})
    if (c_mem_id.value) condition.value["_and"].push({c_mem_id: {"_eq" : c_mem_id.value}})
    if (i_receipt_type.value && is.number(i_receipt_type.value.value)) condition.value["_and"].push({i_receipt_type: {"_eq" : i_receipt_type.value.value}})
    if (status.value) {
    ...
    if (startDate.value) {
      condition.value["_and"].push(
        {d_create: {"_gte" : date.formatDate(date.startOfDate(date.extractDate(startDate.value, "YYYY/MM/DD"), "day"), "YYYY-MM-DDTHH:mm:ss")}}
      )
    }

    if (endDate.value) {
      condition.value["_and"].push(
        {d_create: {"_lte" : date.formatDate(date.endOfDate(date.extractDate(endDate.value, "YYYY/MM/DD"), "day"), "YYYY-MM-DDTHH:mm:ss")}}
      )
    }
  }
