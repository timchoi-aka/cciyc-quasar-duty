...
const eventGalleryPhotoServerPath = "https://www2.cciyc.com/UPFile/ClassGallery/PhotoPic/";
...
export function useClassGalleryPhotoProvider(options = {}) {
      ...
      update: (cache, { data: { delete_HTX_ClassGallery_Photo_by_pk } }) => {
        ...
        if (existingPhotos) {
          ...
          const updatedPhotos = existingPhotos.HTX_ClassGallery_Photo.filter(
            photo => photo.PhotoID !== delete_HTX_ClassGallery_Photo_by_pk.PhotoID
          );
  ...
  const { mutate: updateOrientation, onError: onError_updateOrientation } = useMutation(gql`
    mutation UpdateOrientation($Orientation: Int!, $PhotoID: Int!) {
      update_HTX_ClassGallery_Photo_by_pk(pk_columns: {PhotoID: $PhotoID}, _set: {Orientation: $Orientation}) {
        PhotoID
        Orientation
      }
    }`,
    {
      update: (cache, { data: { update_HTX_ClassGallery_Photo_by_pk } }) => {
        ...
        if (existingPhotos) {
          const updatedPhotos = existingPhotos.HTX_ClassGallery_Photo.map(photo =>
            photo.PhotoID === update_HTX_ClassGallery_Photo_by_pk.PhotoID
              ? { ...photo, Orientation: update_HTX_ClassGallery_Photo_by_pk.Orientation }
              : photo
          );
  ...
  const { mutate: toggleVisibility, onError: onError_toggleVisibility } = useMutation(gql`
    mutation toggleVisibility($IsShow: Int!, $PhotoID: Int!) {
      update_HTX_ClassGallery_Photo_by_pk(pk_columns: {PhotoID: $PhotoID}, _set: {IsShow: $IsShow}) {
        PhotoID
        IsShow
      }
    }`,
    {
      ...
      update: (cache, { data: { update_HTX_ClassGallery_Photo_by_pk } }) => {
        ...
        if (existingPhotos) {
          ...
          const updatedPhotos = existingPhotos.HTX_ClassGallery_Photo.map(photo =>
            photo.PhotoID === update_HTX_ClassGallery_Photo_by_pk.PhotoID
              ? { ...photo, IsShow: update_HTX_ClassGallery_Photo_by_pk.IsShow }
              : photo
          );
  ...
  mutation AddGalleryPhoto($objects: [HTX_ClassGallery_Photo_insert_input!] = {}) {
  ...
  const { mutate: addGalleryPhoto, onError: onError_addGalleryPhoto } = useMutation(ADD_GALLERY_PHOTO_MUTATION, {
    ...
    update: (cache, { data: { insert_HTX_ClassGallery_Photo } }) => {
      // Read the data from our cache for this query.
      const existingPhotos = cache.readQuery({ query: GET_GALLERIES_PHOTOS, variables: params.value.queryVariables }) ?? { HTX_ClassGallery_Photo: [] };

      // Add the new photo to the cache
      const updatedPhotos = [...existingPhotos.HTX_ClassGallery_Photo, ...insert_HTX_ClassGallery_Photo.returning];
