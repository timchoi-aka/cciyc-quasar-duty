        ...
        update: (cache, { data: { delete_HTX_Gallery_Photo_by_pk } }) => {
          ...
          if (existingPhotos) {
            ...
            const updatedPhotos = existingPhotos.HTX_Gallery_Photo.filter(
              (photo) =>
                photo.PhotoID !== delete_HTX_Gallery_Photo_by_pk.PhotoID
            );
    ...
    useMutation(
      gql`
        mutation UpdateOrientation($Orientation: Int!, $PhotoID: Int!) {
          update_HTX_Gallery_Photo_by_pk(
      ...
      `,
      {
        update: (cache, { data: { update_HTX_Gallery_Photo_by_pk } }) => {
          ...
          if (existingPhotos) {
            const updatedPhotos = existingPhotos.HTX_Gallery_Photo.map(
              (photo) =>
                photo.PhotoID === update_HTX_Gallery_Photo_by_pk.PhotoID
                  ? {
                      ...photo,
                      Orientation: update_HTX_Gallery_Photo_by_pk.Orientation,
                    }
                  : photo
            );
  ...
  const { mutate: addGalleryPhoto, onError: onError_addGalleryPhoto } =
    useMutation(ADD_GALLERY_PHOTO_MUTATION, {
      ...
      update: (cache, { data: { insert_HTX_Gallery_Photo } }) => {
        ...
        const updatedPhotos = [
          ...existingPhotos.HTX_Gallery_Photo,
          ...insert_HTX_Gallery_Photo.returning,
        ];
