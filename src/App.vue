<template>
  <q-layout id="q-app" view="hHh lpR lFf">
    ...
    <q-header elevated class="bg-primary text-white" height-hint="98">
      ...
      <MenuBar v-if="isAuthenticated" :key="module" />
    </q-header>
...
import { useQuasar, date as qdate, is } from "quasar";
...
const isInventoryManagement = computed(
  () => $store.getters["userModule/getInventoryManagement"]
);
...
const isAuthenticated = computed(() =>
  is.object($store.getters["userModule/getAuth"])
);
...
const accountList = ref([
  ...
  },
  {
    ...
    enable:
      !isTmp.value &&
      (isInventoryManagement.value || isSystemAdmin.value || isCenterIC.value),
  },
]);
</script>
