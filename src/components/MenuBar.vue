<template><q-tabs v-if="username && module == 'duty'" inline-label class="desktop-only" align="left">
  ...
  <q-route-tab v-if="isUserManagement && !isTmp" to="/user/normal" icon="account_circle" label="用戶管理" />
  <q-route-tab v-if="isSystemAdmin && !isTmp" to="/system-admin" icon="build" label="系統管理" />
  <q-route-tab v-if="(isSystemAdmin || isCenterIC) && !isTmp" to="/bugs" icon="bug_report" label="錯誤報告" />
</q-tabs>
<q-tabs v-if="username && module == 'member' && !isTmp" inline-label class="desktop-only" align="left">
