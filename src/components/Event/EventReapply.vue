  ...
  <q-dialog v-model="reregistrationModal">
    <q-card class="q-ma-none q-pa-none">
      ...
      <q-card-section class="row q-ma-none q-py-none q-px-none">
        ...
        <div class="col-12 q-px-sm">活動日期：{{ qdate.formatDate(qdate.extractDate(props.d_date_from, "D/M/YYYY"), "YYYY年M月D日") + " 至 " + qdate.formatDate(qdate.extractDate(props.d_date_to, "D/M/YYYY"), "YYYY年M月D日") }}</div>
        <div class="col-12 q-px-sm">活動時間：{{ props.d_time_from + " 至 " + props.d_time_to }}</div>
      ...
      <q-card-actions class="row">
        <q-form class="col-12 row"
          @submit="submitApplication"
          >
          <div class="col-12 row">
            <div class="text-h6 text-primary">收費：</div><q-input label="請輸入收費..." outlined lazy-rules :rules="[ val => val && val > 0 || '收費要大於0']" v-model.number="u_fee" type="number" dense/>
            <q-space/>
            <div class="text-h6 text-primary">原因：</div><q-input label="請輸入原因..." outlined lazy-rules :rules="[ val => val && val.length > 0 || '請輸入原因...']" v-model="remark2" type="text" dense/>
          </div>
          <div class="col-12 row">
            <q-space/>
            <div><q-btn label="取消" class="bg-negative text-white q-mr-md" icon="cancel" v-close-popup/></div>
            <div><q-btn label="確認" class="bg-positive text-white" icon="check" type="submit"/></div>
          </div>
        </q-form>
      </q-card-actions>
    </q-card>
...
function submitApplication() {  
  ...
  const logObject = ref({
    ...
    "module": "活動系統",
    "action": "會員 " + props.c_name.trim() + "(" + props.c_mem_id.trim() + ") (重覆)報名活動 " + props.c_act_code.trim() + " 費用: " + u_fee.value,
  })
...
eventReregistration_Completed((result) => {
  loading.value--
  $q.notify({ message: "會員: " + props.c_name + "(重覆)報名活動" + props.c_act_code + "成功。收費 HK$" + result.data.insert_tbl_account_one.u_price_after_discount });
  u_fee.value = 0
