<template>
  ...
  <EventAttendanceReportPDF
    :key="reportMonth"
    :eventData="eventResult"
    :attendanceData="attendanceRecord"
    :attendanceOthersData="attendanceOthersRecord"
...
const { result: attendanceResult } = useAttendanceProvider({
  c_act_code: c_act_code,
});
...
const eventMonths = computed(() => {
  ...
  if (eventResult.value && eventResult.value.HTX_Event_by_pk) {
    let d_start = date.startOfDate(
      date.extractDate(
        eventResult.value.HTX_Event_by_pk.d_date_from,
        "D/M/YYYY"
      ),
      "month"
    );
    let d_end = date.endOfDate(
      date.extractDate(eventResult.value.HTX_Event_by_pk.d_date_to, "D/M/YYYY"),
      "month"
    );
    while (d_start <= d_end) {
...
const buttonOptions = computed(() =>
  eventMonths.value.map((mth) => ({ label: mth, value: mth }))
);
...
const attendanceOthersRecord = computed(() => {
  ...
  if (
    ...
    attendanceResult.value.AttendanceNonRegistrant.forEach((att) => {
      if (date.formatDate(att.d_date, "YYYY-MM") == reportMonth.value) {
        res.push(att);
      }
    });
