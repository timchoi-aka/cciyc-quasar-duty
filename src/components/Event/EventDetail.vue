<template>
  <q-card square>
    ...
    <q-tabs inline-label outside-arrows mobile-arrows align="left" class="bg-blue-10 text-white">
      <q-route-tab icon="source" label="基本資料" :to="{ name: 'EventContent', params: { id: route.params.id } }" />
      <q-route-tab v-if="!isFree" icon="paid" label="費用設定"
        :to="{ name: 'EventFee', params: { id: route.params.id } }" />
      ...
      <q-route-tab icon="report" label="每月服務統計"
        :to="{ name: 'EventAttendanceReport', params: { id: route.params.id } }" />
      <q-route-tab icon="summarize" label="計劃檢討"
        :to="{ path: '/event/detail/' + route.params.id + '/evaluation/view' }" />
