  ...
  <q-card square>
    ...
    >
      <q-route-tab
        icon="source"
        label="基本資料"
        :to="{ name: 'EventContent', params: { id: route.params.id } }"
      />
      <q-route-tab
        v-if="!isFree"
        icon="paid"
      ...
      <q-route-tab
        icon="approval"
        label="報名"
        :to="{ name: 'EventApply', params: { id: route.params.id } }"
      />
      <q-route-tab
        icon="leaderboard"
        label="統計節數"
        :to="{ name: 'EventStat', params: { id: route.params.id } }"
      />
      <q-route-tab
        icon="person_add"
        label="每日統計"
        :to="{ name: 'TakeAttendance', params: { id: route.params.id } }"
      />
      <q-route-tab
        icon="report"
        label="每月服務統計"
