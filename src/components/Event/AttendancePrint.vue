<template class="row items-scretch q-pa-none q-ma-none">
  ...
  <div
    ...
    v-if="!waitingAsync && $q.platform.is.desktop"
  >
...
// variables
const waitingAsync = computed(
  () => eventLoading.value || applicantsLoading.value
);
...
watch([Applicants, Event], ([newApp, newEvent], [oldApp, oldEvent]) => {
  if (newApp.length > 0 && is.object(newEvent))
    generatePDF(
      newApp,
      newEvent,
      numberOfApplicantSlots.value,
      numberOfDateSlots.value
    );
});
...
ApplicantData((result) => {
  if (result.data) {
    Applicants.value = [...result.data.tbl_act_reg].sort(
...
function drawHeader(doc, applicants, event, dateSlots) {
  ...
  if (event.c_act_name)
    doc.text(event.c_act_name, 82, 20, { align: "left", maxWidth: 95 });
  ...
  if (event.d_time_from)
    doc.text(
      qdate.formatDate(
        qdate.extractDate(event.d_time_from, "h:mm:ss A"),
        "h:mm A"
      ),
      172,
      30,
  ...
  if (event.d_time_to)
    doc.text(
      qdate.formatDate(
        qdate.extractDate(event.d_time_to, "h:mm:ss A"),
        "h:mm A"
      ),
      242,
      30,
  ...
  if (event && event.i_lessons)
    doc.text(event.i_lessons.toString(), 200, 40, "left");
...
async function generatePDF(applicants, event, appSlots, dateSlots) {
  ...
  for (let page = 0; page <= applicants.length / appSlots; page++) {
    ...
    for (
      let i = 1;
      i <= Math.min(applicants.length - page * appSlots, appSlots);
      i++
    ) {
      let endOfLine = 62 + ((180 - 62) / appSlots) * i;
      let lineHeight = (180 - 62) / appSlots;
      doc.text(
        (page * appSlots + i).toString(),
        20,
        endOfLine - lineHeight / 2.5,
        "center"
      );
      doc.text(
        applicants[page * appSlots + i - 1].c_name +
          "(" +
          applicants[page * appSlots + i - 1].c_mem_id +
          ")",
        55,
        endOfLine - lineHeight / 2.5,
      ...
      doc.text(
        applicants[page * appSlots + i - 1].i_age.toString(),
        90,
        endOfLine - lineHeight / 2.5,
        "center"
      );
  ...
  $q.platform.is.mobile
    ? openURL(
        doc.output("bloburi", { filename: event.c_act_code + "點名表.pdf" })
      )
