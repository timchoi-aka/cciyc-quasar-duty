...
<div id="printParticipants" class="print-area q-mx-md">
  <div class="row justify-center" style="border-bottom: 2px solid">
    ...
    <div class="row col-12 q-my-sm">
      ...
      <div class="col-3 normal">開始時間：<span style="border-bottom: 1px solid">{{
        qdate.formatDate(qdate.extractDate(Event.d_time_from, "h:mm:ss A"), "h:mm A") }}</span></div>
      <div class="col-3 normal">結束時間：<span style="border-bottom: 1px solid">{{
        qdate.formatDate(qdate.extractDate(Event.d_time_to, "h:mm:ss A"), "h:mm A") }}</span></div>
    </div>
    <div class="row col-12 q-my-sm">
  ...
  <div class="row col-12 q-mt-none" v-for="(member, index) in Applicants" :key="index">
    ...
    <div style="border: 1px solid;" class="col-2 text-center normal">{{ member.EventRegistration_to_Member?.c_tel }}
    </div>
    <div style="border: 1px solid;" class="col-2 text-center normal">{{ qdate.formatDate(member.d_reg, "YYYY-MM-DD") }}
    </div>
