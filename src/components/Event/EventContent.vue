<template>
  ...
  <q-card v-if="$q.screen.gt.xs" bordered flat class="q-pa-none q-ma-none text-h6 fit">
      ...
      <div class="row q-gutter-lg q-ml-sm col-12 justify-start">
        ...
        <span class="col-grow row justify-start">
          <div class="col-12 row">
            <span class="col-auto">活動名稱(中文): </span><q-input v-if="edit" filled type="text" class="col-grow text-h6"
              v-model="editObject.c_act_name" /><span class="text-h6 col-auto" v-else>{{
                Event.c_act_name
              }}</span>
          </div>
          <div class="col-12 row q-mt-sm">
            <span class="col-auto">活動名稱(英文): </span><q-input class="col-grow text-h6" filled v-if="edit" type="text"
              v-model="editObject.c_act_nameen" /><span class="text-h6 col-auto" v-else>{{
                Event.c_act_nameen
              }}</span>
          </div>
        </span>
    ...
    <q-card-section class="row bg-yellow-2 q-pl-none q-pt-none q-pb-lg">
      ...
      <div class="row col-12 q-gutter-lg q-ml-sm">
        <span class="col-3">會計類別：
          <q-select v-if="edit" :options="acc_type" v-model="editObject.c_acc_type" /><span v-else>{{ Event.c_acc_type
            }}</span></span>
        <span class="col-3">狀況:
          <q-select v-if="edit" :options="status" v-model="editObject.c_status" /><span v-else>{{ Event.c_status
            }}</span></span>
        <span class="col-2 column">
          <span class="col">免費:
            <q-checkbox v-if="edit" v-model="editObject.b_freeofcharge" /><span v-else><q-icon class="text-green"
                v-if="Event.b_freeofcharge" name="check" /><q-icon class="text-red" v-else
      ...
      <div class="row col-12 q-gutter-lg q-ml-sm">
        <span class="col-3">類別：
          <q-select v-if="edit" filled :options="type" v-model="editObject.c_type" /><span v-else>{{ Event.c_type
            }}</span></span>
        <span class="col-7">性質：
          <q-select v-if="edit" filled :options="nature" v-model="editObject.c_nature" /><span v-else>{{ Event.c_nature
            }}</span></span>
      </div>
      <div class="row col-12 q-gutter-lg q-ml-sm">
        <span class="col-3">大分類：
          <q-select v-if="edit" filled :options="group1" v-model="editObject.c_group1" /><span v-else>{{ Event.c_group1
            }}</span></span>
        <span class="col-7">細類：
          <q-select v-if="edit" filled :options="group2" v-model="editObject.c_group2" /><span v-else>{{ Event.c_group2
            }}</span></span>
      </div>
      <div class="row col-12 q-gutter-lg q-ml-sm">
        <span class="col-3">對象:
          <SelectionWithAdd v-if="edit" v-model="editObject.c_whojoin" :options="whojoin" />
          <span v-else>{{
            Event.c_whojoin
          }}</span>
        </span>
        <span class="col-3">負責職員:
          <q-select v-if="edit" filled :options="UserList" v-model="editObject.c_respon" /><span v-else>{{
            Event.c_respon }}</span></span>
        <span class="col-3">協助職員:
          <StaffSelectionMultiple :includeTemp="true" v-if="edit" v-model="editObject.c_worker" />
          <span v-else>{{ Event.c_worker }}</span>
        </span>
      </div>
      <div class="row col-12 q-gutter-lg q-ml-sm">
        <span v-if="edit" class="col-6 row items-center q-pt-sm">
          年齡限制:由
        ...
        <span v-else-if="Event.c_age_control" class="col-3">
          年齡限制:
          <span v-if="Event.i_year_from && Event.i_year_to">{{ Event.i_year_from }}歲至{{ Event.i_year_to }}歲</span>
          <span v-else-if="Event.i_year_from && !Event.i_year_to">{{ Event.i_year_from }}歲以上</span>
          <span v-else-if="!Event.i_year_from && Event.i_year_to">{{ Event.i_year_to }}歲以下</span>
        </span>
        <span class="col-4 row items-center q-pt-sm">
          限制方法：
          <q-select class="col-4" v-if="edit" filled :options="['', '報名提示', '才可報名']"
            v-model="editObject.c_age_control" /><span v-else>{{ Event.c_age_control }} </span>
        </span>
      </div>
    </q-card-section>
    ...
    <q-card-section class="row bg-red-1 q-pl-none q-pt-none q-pb-lg">
      ...
      <div class="row col-12 q-gutter-lg q-ml-sm">
        <span class="col-3">名額:
          <q-input v-if="edit" filled type="number" v-model="editObject.i_quota_max" /><span v-else>{{ Event.i_quota_max
            }}</span></span>
        <span class="col-3">總堂數:
          <q-input v-if="edit" filled type="number" v-model="editObject.i_lessons" /><span v-else>{{ Event.i_lessons
            }}</span></span>
        <span class="col-3">逢星期:
          <SelectionWithAdd v-if="edit" v-model="editObject.c_week" :options="c_week_options" /><span v-else>{{
            Event.c_week }}</span>
        </span>
      </div>
    </q-card-section>
    ...
    <q-card-section class="row col-12 bg-brown-1 q-pl-none q-pt-none q-pb-lg">
      <div class="col-6 row items-start content-start">
        ...
        <div class="row col-12 q-gutter-lg q-ml-sm items-start">
          <span class="col-11">舉行地點:
            <SelectionWithAdd v-if="edit" v-model="editObject.c_dest" :options="destOptions" /><span v-else>{{
              Event.c_dest }}</span>
          </span>
          <span class="col-11">集合地點:
            <SelectionWithAdd v-if="edit" v-model="editObject.c_start_collect" :options="destOptions" /><span v-else>{{
              Event.c_start_collect
              }}</span>
          </span>
          <span class="col-11">解散地點:
            <SelectionWithAdd v-if="edit" v-model="editObject.c_end_collect" :options="destOptions" /><span v-else>{{
              Event.c_end_collect
              }}</span>
          </span>
        </div>
      ...
      <div class="col-6 row items-start content-start">
        ...
        <div class="row col-12 q-gutter-lg q-ml-sm">
          <span class="col-12 row">顯示網頁:
            <q-checkbox v-if="edit" v-model="editObject.IsShow" /><span v-else><q-icon class="text-green"
                v-if="Event.IsShow" name="check" /><q-icon class="text-red" v-else name="cancel" /></span></span>
  ...
  <div v-else>
    ...
    <q-expansion-item header-class="bg-grey-4 text-body1">
      ...
      <div class="row q-px-sm col-12 justify-start text-body1">
        ...
        <span class="col-12 row justify-start items-start">
          <div class="col-12 row">
            <span class="col-auto">活動名稱(中文): </span><q-input v-if="edit" filled type="text" class="col-grow"
              v-model="editObject.c_act_name" /><span class="col-auto" v-else>{{ Event.c_act_name }}</span>
          </div>
          <div class="col-12 row">
            <span class="col-auto">活動名稱(英文): </span><q-input class="col-grow" filled v-if="edit" type="text"
              v-model="editObject.c_act_nameen" /><span class="col-auto" v-else>{{ Event.c_act_nameen }}</span>
          </div>
        </span>
      </div>
    </q-expansion-item>
    ...
    <q-expansion-item header-class="bg-yellow-4 text-body1">
      ...
      <div class="row col-12 q-px-sm text-body1">
        <span class="col-12">會計類別：
          <q-select v-if="edit" :options="acc_type" v-model="editObject.c_acc_type" /><span v-else>{{ Event.c_acc_type
            }}</span></span>
        <span class="col-12">狀況:
          <q-select v-if="edit" :options="status" v-model="editObject.c_status" /><span v-else>{{ Event.c_status
            }}</span></span>
        ...
        <span class="col-12">達標日期:
          <span v-if="edit">
            <DateComponent v-model="editObject.d_finish_goal" />
          </span><span v-else>{{
            qdate.formatDate(Event.d_finish_goal, "YYYY年M月D日")
          }}</span></span>
        <span class="col-12">類別：
          <q-select v-if="edit" filled :options="type" v-model="editObject.c_type" /><span v-else>{{ Event.c_type
            }}</span></span>
        <span class="col-12">性質：
          <q-select v-if="edit" filled :options="nature" v-model="editObject.c_nature" /><span v-else>{{ Event.c_nature
            }}</span></span>
        <span class="col-12">大分類：
          <q-select v-if="edit" filled :options="group1" v-model="editObject.c_group1" /><span v-else>{{ Event.c_group1
            }}</span></span>
        <span class="col-12">細類：
          <q-select v-if="edit" filled :options="group2" v-model="editObject.c_group2" /><span v-else>{{ Event.c_group2
            }}</span></span>
        <span class="col-12">對象:
          <SelectionWithAdd v-if="edit" v-model="editObject.c_whojoin" :options="whojoin" />
          <span v-else>{{ Event.c_whojoin }}</span>
        </span>
        <span class="col-12">負責職員:
          <q-select v-if="edit" filled :options="UserList" v-model="editObject.c_respon" /><span v-else>{{
            Event.c_respon
          }}</span></span>
        ...
        <span class="col-12" v-if="edit">限制方法:
          <q-select v-if="edit" :options="['', '報名提示', '才可報名']" v-model="editObject.c_age_control" />
        </span>
        <span v-else>限制方法：{{ Event.c_age_control }}</span>

        <span class="col-12 row items-center" v-if="edit">年齡限制:由
          <q-input class="col-3" type="number" v-model="editObject.i_year_from" />
          歲至
          <q-input class="col-3" type="number" v-model="editObject.i_year_to" />歲
        </span>
    ...
    <q-expansion-item header-class="bg-red-3 text-body1">
      ...
      <div class="row col-12 q-px-sm text-body1">
        <span class="col-12">開始日期:
          <span v-if="edit">
            <DateComponent v-model="editObject.d_date_from" />
          </span><span v-else>{{ Event.d_date_from }}</span></span>
        <span class="col-12">開始時間:
          <span v-if="edit">
            <TimeComponent v-model="editObject.d_time_from" />
          </span><span v-else>{{ Event.d_time_from }}</span></span>
        <span class="col-12">報名日期(開始):
          <span v-if="edit">
            <DateComponent v-model="editObject.d_sale_start" />
          </span><span v-else>{{ Event.d_sale_start }}</span></span>
        <span class="col-12">終結日期:
          <span v-if="edit">
            <DateComponent v-model="editObject.d_date_to" />
          </span><span v-else>{{ Event.d_date_to }}</span></span>
        <span class="col-12">終結時間:
          <span v-if="edit">
            <TimeComponent v-model="editObject.d_time_to" />
          </span><span v-else>{{ Event.d_time_to }}</span></span>
        <span class="col-12">報名日期(完結):
          <span v-if="edit">
            <DateComponent v-model="editObject.d_sale_end" />
          </span><span v-else>{{ Event.d_sale_end }}</span></span>
        <span class="col-12">名額:
          <q-input v-if="edit" filled type="number" v-model="editObject.i_quota_max" /><span v-else>{{ Event.i_quota_max
            }}</span></span>
        <span class="col-12">總堂數:
          <q-input v-if="edit" filled type="number" v-model="editObject.i_lessons" /><span v-else>{{ Event.i_lessons
            }}</span></span>
        <span class="col-12">逢星期:
          <SelectionWithAdd v-if="edit" v-model="editObject.c_week" :options="c_week_options" /><span v-else>{{
            Event.c_week
          }}</span>
        </span>
  ...
  <q-page-sticky v-if="$q.screen.lt.sm" position="bottom-right" :offset="[20, 20]" style="z-index: 1">
    <q-fab unelevated color="primary" icon="keyboard_arrow_up" direction="up">
      ...
      <q-fab-action v-if="edit" label="儲存" icon="save" class="bg-white text-positive" push
