  ...
  >
    ...
    <template v-slot:selected>
      <div v-if="props.modelValue">
        {{ props.modelValue.value }} - ${{ props.modelValue.u_fee }}
      </div>
    </template>
...
const { onResult } = useQuery(gql`
  query getIncomeType {
    tbl_sel_acc_Income_type(
      where: { b_disable: { _eq: false } }
      order_by: { i_No: asc }
    ) {
      b_fixed
      c_acc_type
...
function incomeFilter(val, update) {
  ...
  update(() => {
    IncomeOptions.value = OriginalIncomeOptions.value;
    IncomeOptions.value = IncomeOptions.value.filter(
      (v) =>
        (v.value ? v.value.indexOf(val) > -1 : false) ||
        (v.c_type
          ? v.c_type.toLowerCase().indexOf(val.toLowerCase()) > -1
          : false) ||
        (v.c_type_no
          ? v.c_type_no.toLowerCase().indexOf(val.toLowerCase()) > -1
          : false)
    );
  });
}
