<template>
  <div>
    ...
    <q-dialog v-model="showPrintVoucher">
      <VoucherPDF :data="voucherObject" type="預支" />
    </q-dialog>

    <q-card v-if="prepaidResult.length" class="row">
      <q-card-section
        class="text-body2 bg-grey-2 text-left text-bold col-12 q-ma-none q-pa-sm"
        >預支記錄(共：${{
          prepaidResult
            .filter((x) => x.approved || (!x.approved && !x.approve_date))
            .reduce((a, v) => (a += v.amount), 0)
        }}) - 預支上限：${{
          parseFloat(total * 0.8).toFixed(1)
        }}</q-card-section
      >
      <q-card-section class="row fit justify-left q-ma-none q-pa-sm">
        <div
          class="col-12 row items-center content-start prepaid-item"
          v-for="(record, index) in prepaidResult"
          :key="index"
        >
          <span class="col-4 text-left">{{
            qdate.formatDate(record.apply_date, "YYYY年M月D日")
          }}</span>
          ...
          <span class="col-2"
            ><q-chip
              ...
              dense /><q-chip
              v-if="!record.approved && record.approve_date"
              label="拒批"
              dense
              class="bg-red text-white" /><q-chip
              v-if="!record.approved && !record.approve_date"
              label="未批"
              class="bg-warning text-white"
              dense
          /></span>
          <span v-if="record.approved" class="col-2"
            ><q-btn
              size="sm"
              icon="print"
              class="bg-primary text-white"
              @click="printVoucher(prepaidResult[index])"
          /></span>
          <q-space />
          <span
            class="col-1"
            v-if="
              record.apply_user.trim() == username &&
              !record.approved &&
              record.approve_date
            "
            ><q-btn
              ...
              dense
              icon="edit"
              @click="prepaid = JSON.parse(JSON.stringify(record))"
          /></span>
          <span
            class="col-1"
            v-if="
            ...
            ><q-btn
              ...
              dense
              icon="delete"
              @click="deletePrepaid(record.uuid)"
          /></span>
          <span
            class="col-3"
            v-if="isCenterIC && !record.approved && !record.approve_date"
            ><q-btn
              class="bg-white text-red"
              flat
              dense
              icon="close"
              @click="rejectPrepaid(record.uuid)" /><q-btn
              class="bg-white text-secondary"
              flat
              dense
              icon="check"
              @click="acceptPrepaid(record.uuid)"
          /></span>
          <q-tooltip
            v-if="record.recipient && record.recipient.trim().length > 0"
            class="bg-grey-4 text-black text-body1"
            :offset="[10, 10]"
          >
    ...
    <q-btn
      v-if="
        Object.keys(prepaid).length == 0 &&
        prepaidResult.length == 0 &&
        (props.respon.includes(username) || isUAT) &&
        !props.isSubmitted
      "
      class="bg-primary text-white q-my-sm"
      icon="money"
      label="申請預支"
      @click="prepaid = { recipient: '', amount: 0 }"
    />
    <q-form @submit="save" @reset="prepaid = {}">
      <div class="row" v-if="Object.keys(prepaid).length">
                ...
                (val > 0 &&
                  val <=
                    total * 0.8 -
                      prepaidResult
                        .filter(
                          (x) => x.approved || (!x.approved && !x.approve_date)
                        )
                        .reduce((a, v) => a + v.amount, 0)) ||
                '預支為0或超過上限',
            ]"
        ...
        <div class="col-12 text-right q-mt-sm row">
          ...
          />
          <q-btn
            class="bg-secondary text-white text-right q-mx-sm"
            flat
            label="遞交申請"
            type="submit"
          />
...
import VoucherPDF from "src/components/HealthCare/VoucherPDF.vue";
...
// queries
const { onResult: getEvaluationResult } = useQuery(
  gql`
...
const {
  result: getPrepaidResult,
  onError: getPrepaidResult_Error,
  refetch: getPrepaidResult_Refetch,
} = useQuery(
  gql`
...
const {
  mutate: addPrepaidRequest,
  onDone: addPrepaidRequest_Completed,
  onError: addPrepaidRequest_Error,
} = useMutation(gql`
...
const {
  mutate: editPrepaidRequest,
  onDone: editPrepaidRequest_Completed,
  onError: editPrepaidRequest_Error,
} = useMutation(gql`
...
const { mutate: approvePrepaid, onDone: approvePrepaid_Completed } =
  useMutation(gql`
...
const prepaidResult = computed(() =>
  getPrepaidResult.value ? getPrepaidResult.value.Event_Prepaid : []
);
...
function save() {
  let prepaidObject = {
    ...
    eval_uuid: props.eval_uuid,
    type: "預支",
    payment_method: prepaid.value.payment_method
      ? prepaid.value.payment_method.trim()
      : null,
  };

  if (prepaid.value.uuid) {
    // edit record
    let logObject = {
...
function acceptPrepaid(uuid) {
  ...
  $q.dialog({
    title:
      '<div class="bg-blue text-white q-pa-sm q-ma-none">批核預支申請</div>',
    message:
      "<div class='text-body1 q-py-xs'>確定批核預支？ </div><br/><div>申請人：" +
      prepaidResult.value[i].apply_user +
      "</div><br/><div>活動：" +
      prepaidResult.value[i].c_act_code +
      "</div><br/><div>金額：$" +
      prepaidResult.value[i].amount.toFixed(1) +
      "</div><hr class='q-mb-md'/><div class='text-body1'>選擇支付方法：</div>",
    html: true,
    options: {
      type: "radio",
...
function rejectPrepaid(uuid) {
  ...
  $q.dialog({
    title:
      '<div class="bg-blue text-white q-pa-sm q-ma-none">拒絕預支申請</div>',
    message:
      "<div class='text-body1 q-py-xs'>確定拒絕預支？ </div><br/><div>申請人：" +
      prepaidResult.value[i].apply_user +
      "</div><br/><div>活動：" +
      prepaidResult.value[i].c_act_code +
      "</div><br/><div>金額：$" +
...
getEvaluationResult((result) => {
  if (result.data) {
    total.value = result.data.Event_Evaluation_Account.reduce(
      (a, v) => a + v.amount,
      0
    );
  }
...
addPrepaidRequest_Completed((result) => {
  ...
  $q.notify({
    message:
      "活動" +
      result.data.insert_Event_Prepaid_one.c_act_code +
      "，成功新增預支記錄。",
  });
});
...
editPrepaidRequest_Completed((result) => {
  ...
  $q.notify({
    message:
      "活動" +
      result.data.update_Event_Prepaid_by_pk.c_act_code +
      "，成功修改預支記錄。",
  });
});
