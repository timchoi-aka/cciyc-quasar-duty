<template>
  ...
  <q-dialog v-model="upload" position="bottom">
    <FileUpload style="width: 300px;" class="col-12" mode="multiple" :path="'News/' + props.newsID" @onDone="(val) => filename.push(fileprefix + 'News/' + props.newsID + '/' + val.files[0].name)"/>
  </q-dialog>

  <!-- insert image dialog -->
  <q-dialog v-model="imgSelector" position="bottom" style="width: 100%; overflow: auto;">
    <q-carousel
      animated
      ...
      <template v-for="(i, index) in filename">
        <q-carousel-slide :name="index" :img-src="i" >
          <div class="absolute-top-right text-subtitle2">
            <q-btn icon="add" label="插入圖片" class="bg-grey-3" outline @click="insertFilenames(i, imageSize)"/>
          </div>
...
function insertFilenames(filename, size) {
  let insertTag = "<img src='" + filename + "' style='max-width: 800px; width: " + size + "%;'/>"
