...
<template>
  ...
  <q-dialog v-model="confirmDialog">
    <q-card style="width: 70vw; border-radius: 30px" class="q-gutter-md q-pa-md">
      <q-card-section>
        <div class="text-h5 bg-blue-3 text-center">確定申請放假？</div>
      </q-card-section>

      <q-card-section
        class="scroll row items-begin bg-grey-2 q-gutter-md q-pa-md"
        :style="
          $q.screen.lt.sm
        ...
        "
      >
        <div v-if="applicationList.length > 0" class="q-pa-md col-6">
          <div class="text-h5 text-center">新增</div>
          <div
        ...
        <q-separator vertical class="col-auto" />
        <div v-if="deleteApplicationList.length > 0" class="q-pa-md col-6">
          <div class="text-h5 text-center">刪除</div>
          <div
            class="row wrap justify-center items-start content-start"
        ...
        <div v-if="changeApplicationList.length > 0" class="q-pa-md col-6">
          <div class="text-h5 text-center">修改</div>
          <q-card
  ...
  <!-- toolbar -->
  <div class="full-width row no-wrap justify-center content-center q-px-sm toolbar">
    <q-btn
      class="col-md-1 col-lg-1 col-xl-1 col-xs-2 col-sm-2"
      size="lg"
      outline
      color="primary"
    ...
    <div class="q-ma-md col-md-1 col-lg-1 col-xl-1 col-sm-2 col-xs-2">
      年假結餘
      <q-knob
        :min="0"
        :max="40"
    ...
    <div v-if="isSAL" class="col-md-1 col-lg-1 col-xl-1 col-sm-2 col-xs-2">
      特別結餘
      <q-knob
        :min="0"
        readonly
      ...
      >
        ...
        <template v-slot:body-cell-Date="props">
          <q-td :class="['nameColumn', getHoliday(props.row.Date) ? 'isHoliday' : '']"
            >{{ props.value }}
            <span class="bg-red-2" v-if="getHoliday(props.row.Date)">
        ...
        <template v-slot:body-cell="props">
          <q-td
            ...
            @click="props.row[props.col.name + '.approved'] ? '' : applyHoliday(props)"
...
export default defineComponent({
  ...
  methods: {
    ...
    async refreshHolidayTable() {
      ...
      let queryEndDate = qdate.endOfDate(queryStartDate, "year");
