  ...
  <q-dialog v-model="renewDialog">
    <q-card>
      <q-card-section class="bg-primary text-white text-h6">
        ...
        <span v-if="renewObject.duration == 1">(1年)</span
        ><span v-else>(永久)</span>
      </q-card-section>
  ...
  <q-card>
    <q-card-section class="bg-primary text-white text-h6">
      ...
      <div class="row" v-if="$q.screen.lt.sm">
        ...
        <q-btn
          v-if="!editState"
          class="text-white q-mr-none col-shrink"
          rounded
          flat
          icon="edit"
          @click="startEdit"
      ...
      <div class="row" v-else>
        <div class="col-*">
          ...
          <span v-if="member && member.c_name_other" class="col-1"
            >({{ member && member.c_name_other }})</span
          >
      ...
      <div class="row">
        ...
        <div class="row col-xs-12">
          <q-btn
            v-if="isSystemAdmin || isCenterIC"
            class="bg-white bg-red"
            dense
            flat
            icon="delete"
            label="刪除"
          ...
          >
            <q-list>
              <q-item
                clickable
                v-close-popup
                @click="renewMemberModal(member, 1)"
              >
                <q-item-section>
              ...
              <q-item
                clickable
                v-close-popup
                @click="renewMemberModal(member, 999)"
              >
                <q-item-section>
    ...
    >
      ...
      >
        <div class="col-12 col-xs-12 row">
          <span class="col-md-2 col-sm-2 col-xs-4">中文姓名:</span
          ><span v-if="editState" class="col-md-10 col-sm-10 col-xs-8"
            ><q-input filled v-model="edit_member.c_name" /></span
          ><span v-else class="col-md-10 col-sm-10 col-xs-8">{{
            member && member.c_name
          }}</span>
        </div>
        <div class="col-12 col-xs-12 row">
          <span class="col-md-2 col-sm-2 col-xs-4">英文姓名:</span
          ><span v-if="editState" class="col-md-10 col-sm-10 col-xs-8"
            ><q-input filled v-model="edit_member.c_name_other" /></span
          ><span v-else class="col-md-10 col-sm-10 col-xs-8">{{
        ...
        <div class="col-12 col-xs-12 row">
          ...
          <span v-if="editState" class="col-md-10 col-sm-10 col-xs-8">
            ...
            /> </span
          ><span v-else>{{ member && member.c_sex }}</span>
        </div>
        ...
        <div class="col-12 row">
          <span class="col-md-2 col-sm-2 col-xs-4">年齡: </span
          ><span v-if="editState" class="col-md-10 col-sm-10 col-xs-8"
            ><!--{{
        ...
        <div class="col-12 row">
          <span class="col-md-2 col-sm-2 col-xs-4">手提電話: </span
          ><span v-if="editState" class="col-md-10 col-sm-10 col-xs-8"
            ><q-input
              filled
              v-model="edit_member.c_mobile"
              mask="########" /></span
          ><span v-else class="col-md-10 col-sm-10 col-xs-8">{{
        ...
        <div class="col-12 row">
          <span class="col-md-2 col-sm-2 col-xs-4">地址:</span
          ><span v-if="editState" class="col-md-10 col-sm-10 col-xs-8"
            ><q-input filled v-model="edit_member.m_addscom" /></span
          ><span v-else class="col-md-10 col-sm-10 col-xs-8">{{
            member && member.m_addscom
          }}</span>
        </div>
        <div class="col-12 row">
          <span class="col-md-2 col-sm-2 col-xs-4">電郵:</span
          ><span v-if="editState" class="col-md-10 col-sm-10 col-xs-8"
            ><q-input filled v-model="edit_member.c_email" /></span
          ><span v-else class="col-md-10 col-sm-10 col-xs-8">{{
    ...
    <q-card-section
      class="bg-yellow-1 row q-pa-none"
      style="border: 1px solid lightgrey"
    >
      ...
      >
        <div class="col-12 row">
          <span class="col-md-2 col-sm-2 col-xs-4">緊急聯絡人: </span
          ><span v-if="editState" class="col-md-10 col-sm-10 col-xs-8"
            ><q-input filled v-model="edit_member.c_emer_name" /></span
          ><span v-else class="col-md-10 col-sm-10 col-xs-8">{{
            member && member.c_emer_name
          }}</span>
        </div>
        <div class="col-12 row">
          <span class="col-md-2 col-sm-2 col-xs-4">關係: </span
          ><span v-if="editState" class="col-md-10 col-sm-10 col-xs-8"
            ><q-input filled v-model="edit_member.c_emer_rel" /></span
          ><span v-else class="col-md-10 col-sm-10 col-xs-8">{{
            member && member.c_emer_rel
          }}</span>
        </div>
        <div class="col-12 row">
          <span class="col-md-2 col-sm-2 col-xs-4">電話: </span
          ><span v-if="editState" class="col-md-10 col-sm-10 col-xs-8"
            ><q-input
              filled
              v-model="edit_member.c_emer_tel1_1"
              mask="########" /></span
          ><span v-else class="col-md-10 col-sm-10 col-xs-8">{{
    ...
    <q-card-section
      class="bg-green-1 row q-pa-none"
      style="border: 1px solid lightgrey"
    >
      ...
      >
        <div class="col-12 row">
          ...
          <span v-else class="col-md-10 col-sm-10 col-xs-8"
            >{{ member && member.c_udf_1 }}(<q-icon
              v-if="member && member.b_mem_type1"
              color="positive"
              name="check"
            />
        ...
        <div class="col-12 row">
          ...
          <span class="col-md-2 col-sm-2 col-xs-4">退會日期:</span
          ><span class="col-md-10 col-sm-10 col-xs-8">{{
            qdate.formatDate(member ? member.d_exit_1 : null, "YYYY年MM月DD日")
          }}</span>
          <span class="col-md-2 col-sm-2 col-xs-4">青年家人:</span
          ><span class="col-md-10 col-sm-10 col-xs-8">
            <q-icon
              v-if="member && member.b_mem_type10"
              color="positive"
              name="check"
            />
            <q-icon v-else color="negative" name="cancel" />
      ...
      >
        <div v-if="editState" class="full-width row">
          <div class="full-width row">
            ...
            <span class="col-1 q-mr-md-md q-mr-sm-sm q-mr-xs-none"
              >會藉有效</span
            >
            <span class="col-2 q-mr-md-md q-mr-sm-sm q-mr-xs-none"
              >關聯日期</span
            >
          ...
          <div
            class="row col-12"
            v-for="(relation, index) in edit_relationTable"
            :key="index"
          >
            <span v-if="!relation.delete" class="col-12">
              <MemberRelated
                :key="props.modelValue"
                v-model="edit_relationTable[index]"
                class="row fit"
        ...
        <div v-else>
          ...
          >
            ...
            <span v-if="!relation.delete">
              ...
              <q-chip
                >關聯日期：{{
                  qdate.formatDate(relation.d_effective, "YYYY年M月D日")
                }}</q-chip
              >
            </span>
          </div>
    ...
    <q-card-section
      class="bg-grey-1 row text-caption justify-end q-pa-none"
      style="border: 1px solid lightgrey"
    >
...
<script setup>
import { computed, ref, watch, onMounted, toRef, unref, isRef } from "vue";
import { useStore } from "vuex";
import LoadingDialog from "components/LoadingDialog.vue";
import { date as qdate, is, useQuasar, uid, extend } from "quasar";
import {
  DELETE_MEMBER_BY_ID,
  UPDATE_RELATED_YOUTH_MEMBER_STATUS,
  QUIT_MEMBER_BY_ID,
  UPDATE_MEMBER_BY_ID,
  UPDATE_YOUTH_MEMBER_STATUS,
  INSERT_RELATION,
  UPDATE_RELATION,
  DELETE_RELATION,
  RENEW_MEMBER_FROM_ID_WITH_PAYMENT,
} from "/src/graphQueries/Member/mutation.js";
import {
  GET_RELATED_MEMBER_FROM_ID,
  LATEST_RECEIPT_NO,
...
import { extractRelationChange, checkMemberRecord } from "src/lib/member.js";
...
const c_mem_id = toRef(props, "modelValue");
// emits
...
const { result: member, refetch } = useMemberProvider({
  ...
  loadDetail: ref(true),
  loadMembership: ref(true),
  loadHousekeep: ref(true),
...
watch(edit_relationTable, (newVal, oldVal) => {
  ...
  if (newVal.length > 0) {
    ...
    updateQueue.value = newQueue;
  }
});

const {
  mutate: QuitMember,
  onDone: QuitMember_Completed,
  onError: QuitMember_Error,
} = useMutation(QUIT_MEMBER_BY_ID, { errorPolicy: "all" });
const {
  mutate: UpdateMember,
  onDone: UpdateMember_Completed,
  onError: UpdateMember_Error,
} = useMutation(UPDATE_MEMBER_BY_ID);
const {
  mutate: UpdateYouthMemberStatus,
  onDone: UpdateYouthMemberStatus_Completed,
  onError: UpdateYouthMemberStatus_Error,
} = useMutation(UPDATE_YOUTH_MEMBER_STATUS);
const {
...
const {
  mutate: RemoveMember,
  onDone: RemoveMember_Completed,
  onError: RemoveMember_Error,
} = useMutation(DELETE_MEMBER_BY_ID);
const {
  mutate: RenewMember,
  onDone: RenewMember_Completed,
  onError: RenewMember_Error,
} = useMutation(RENEW_MEMBER_FROM_ID_WITH_PAYMENT);
...
const relationTable = computed(() => {
  ...
  if (member.value) {
    let relations = [
      ...member.value.MemberRelation1,
      ...member.value.MemberRelation2,
    ];
    for (const rel of relations) {
      if (Object.keys(rel).length > 0) {
        if (rel.c_mem_id_1 == currentMemberID.value && rel.RelationMember2) {
          // c_mem_id_2 is the related member
          result.push({
            c_mem_id_1: rel.c_mem_id_1,
            c_mem_id_2: rel.c_mem_id_2,
            relation: rel.relation,
            name: rel.RelationMember2
              ? rel.RelationMember2.c_name
                ? rel.RelationMember2.c_name
                : rel.RelationMember2.c_name_other
              : "",
            age: rel.RelationMember2
              ? ageUtil.calculateAge(rel.RelationMember2.d_birth)
              : 0,
            d_birth: rel.RelationMember2 ? rel.RelationMember2.d_birth : "",
            b_mem_type1: rel.RelationMember2
              ? rel.RelationMember2.b_mem_type1
              : false,
            c_udf_1: rel.RelationMember2 ? rel.RelationMember2.c_udf_1 : "",
          ...
          //c_mem_id_1 is the related member
          result.push({
            c_mem_id_1: rel.c_mem_id_1,
            c_mem_id_2: rel.c_mem_id_2,
            relation: rel.relation,
            name: rel.RelationMember1
              ? rel.RelationMember1.c_name
                ? rel.RelationMember1.c_name
                : rel.RelationMember1.c_name_other
              : "",
            age: rel.RelationMember1
              ? ageUtil.calculateAge(rel.RelationMember1.d_birth)
              : 0,
            d_birth: rel.RelationMember1 ? rel.RelationMember1.d_birth : "",
            b_mem_type1: rel.RelationMember1
              ? rel.RelationMember1.b_mem_type1
              : false,
            c_udf_1: rel.RelationMember1 ? rel.RelationMember1.c_udf_1 : "",
...
function startEdit() {
  edit_member.value = new Member(member.value);
  extend(true, edit_relationTable.value, relationTable.value);
...
async function saveRecord() {
  ...
  const { status: memberValid, message: memberMessage } = checkMemberRecord(
    edit_member.value
  );

  // display all warning/error messages
  for (let i = 0; i < memberValid.length; i++) {
    if (memberValid[i] == 2) recordValid = false;
    $q.notify({
      message: memberMessage[i],
      icon: memberValid[i] == 1 ? "warning" : "error",
      textColor: "white",
      color: memberValid[i] == 1 ? "warning" : "negative",
    });
  }
  ...
  const {
    ...
    changeRelation,
    newRelation,
  } = extractRelationChange(relationTable.value, edit_relationTable.value);
  ...
  */
  const updateObject = ref({
    ...
    c_udf_1: is.object(edit_member.value.c_udf_1)
      ? edit_member.value.c_udf_1.value
      : edit_member.value.c_udf_1,
  ...
  const logObject = ref({
    ...
    action:
      ...
      " 新資料:" +
      JSON.stringify(updateObject.value) +
      " 舊關係:" +
      JSON.stringify(relationTable.value) +
      " 新關係:" +
      JSON.stringify(newRelation) +
      " 更改關係:" +
      JSON.stringify(changeRelation) +
      " 刪除關係:" +
  ...
  switch (member.c_udf_1) {
    case "個人會員":
      if (duration == 1) {
        ...
        expiryDate = qdate.addToDate(member.d_expired_1, {
          years: 1,
          hours: 8,
        });
      } else {
...
function renewMember(renewObject) {
  const remark =
    renewObject.duration == 1
      ? "繳 付：至" +
        qdate.formatDate(renewObject.new_expired_1, "YYYY年MM月") +
        "之會費\r\n屆滿日期:" +
        qdate.formatDate(renewObject.new_expired_1, "DD/MM/YYYY")
      : "繳 付：永久會員會費";
  ...
  const logObject = ref({
    ...
    action:
      "會員續會" +
      renewDuration +
      renewObject.c_mem_id +
      " 會藉: " +
      renewObject.c_udf_1 +
      " 新屆滿日期: " +
      qdate.formatDate(renewObject.new_expired_1, "YYYY年MM月DD日") +
      " 費用: " +
      renewObject.renewFee,
  });
  ...
  const accountObject = ref({
    ...
    c_desc:
      renewObject.duration == 1 ? "會員續會費" : "永久會員(會員升級)續會費",
    c_type: "續會員費",
    u_discount: 0,
    ...
    c_name: renewObject.c_name
      ? renewObject.c_name.trim()
      : renewObject.c_name_other.trim(),
    b_cssa: false,
    b_refund: false,
    b_OtherIncome: false,
    b_clear: false,
    ...
    $q.notify(
      {
        ...
        message:
          "會員: " +
          result.data.update_Member_by_pk.c_mem_id +
          "續會成功. 續會日期：" +
          qdate.formatDate(Date.now(), "YYYY年MM月DD日"),
        actions: [
          {
            label: "列印收據",
            color: "yellow",
            handler: () => {
              printReceiptMember.value =
                result.data.update_Member_by_pk.c_mem_id;
...
RemoveMember_Completed((result) => {
  ...
  if (deleteRelateMember.returning.length > 0) {
    $q.notify({
      message:
        "刪除會員編號: " +
        deleteMember.c_mem_id +
        " 成功，" +
        deleteRelateMember.returning.length +
        "個關聯關係已刪除。",
    });
...
GetRelationByPK_Completed((result) => {
  ...
  if (result.data.Member_by_pk) {
    ...
    let currentExpiryDate = result.data.Member_by_pk.d_expired_1;
    ...
    if (rel.length > 0) {
      ...
      rel.forEach((rm) => {
        if (rm.c_mem_id_1 == result.data.Member_by_pk.c_mem_id) {
          ...
          // only check if this member is not a youth family member
          if (!isYouth) {
            isYouth =
          ...
          if (isYouth && youthMembership) {
            ...
            /* console.log(
              "is youth, family membership:" + rm.RelationMember2.c_udf_1
            ); */
...
QuitMember_Completed((result) => {
  refetch();
  loading.value--;
  let validityResult = result.data.update_Member_by_pk.b_mem_type1
    ? "有效"
    : "無效";
  $q.notify({
    message:
      ...
      " 退會日期：" +
      qdate.formatDate(
        result.data.update_Member_by_pk.d_exit_1,
        "YYYY年MM月DD日"
      ),
  });
  if (relationTable.value.length > 0) {
    relationTable.value.forEach((rel) => {
      if (result.data.update_Member_by_pk.c_mem_id == rel.c_mem_id_1)
        updateQueue.value.push(rel.c_mem_id_2);
...
UpdateMember_Completed((result) => {
  ...
  if (newRelation.returning.length > 0) {
    newRelation.returning.forEach((rel) => {
      if (rel.uuid != "00000000-0000-0000-0000-000000000000") {
        // not the dummy record
  ...
  if (result.data.update_Member_by_pk.c_mem_id) {
    ...
    $q.notify({
      message:
        "編號: " + result.data.update_Member_by_pk.c_mem_id + "更新資料成功.",
    });
  }
  editState.value = false;
  loading.value--;
