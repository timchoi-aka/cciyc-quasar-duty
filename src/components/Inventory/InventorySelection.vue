...
// query
const { result: InventoryResult } = useInventoryProvider();

watch(InventoryResult, (newResult) => {
  if (newResult.data) {
    InventoryOptions.value = [];
    newResult.data.Inventory.forEach((d) => {
      if (d.ID != props.InventoryID) {
...
function inventoryFilter(val, update) {
  ...
  update(() => {
    InventoryOptions.value = OriginalInventoryOptions.value;
    InventoryOptions.value = InventoryOptions.value.filter(
      (v) =>
