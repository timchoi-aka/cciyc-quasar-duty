<template>
  <div class="row q-ma-md-none q-pa-md-none bg-grey-3 justify-center">
    ...
    <div id="printMe" class="print-area bg-white items-start">
...
function convertToChinese(number) {
  ...
  const chineseParts = [];
  const digitsPart = [];
  hundred = Math.floor(unit/100)
  if (unit >= 100) {
    digitsPart.push(digits[Math.floor(unit / 100)] + '佰');
    unit %= 100;
  }
  ten = Math.floor(unit/10)
  if (unit >= 10) {
    if (Math.floor(unit / 10) != 1) {
      digitsPart.push(digits[Math.floor(unit / 10)] + '拾');
    } else {
      if (hundred > 0)
        digitsPart.push('壹拾');
